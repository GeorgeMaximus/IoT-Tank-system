...
* This is software created after shipping the first and second prototype. It should use Threading to achieve some tasks.
* This code was supposed to be delivered and working by April 5th but has not.
* This is an attempt to take this non-working code and get it to work.
* This code should sswap between WiFi and Ethernet if one or the other fails.
* 
...
 * Author: George Raouf
 * Created on 1/8/2021
 * modified D.Bennett on 10th Jan 
 * 
 *  TO DO
 *  1. Pressure Sensor needs new calibration
 *  2. Cause of crash needs resolving
 *  3. Need to be able to create AP to enter new WiFi if it crashes.
 *  4. Check Si7021 works with new sensor.
 * 
 * System Sensors: -
...
char dataStr[100] = "";      // holds stringod data writen to CSV file
String CSVdata = "";        // string vaiable of CSV data
const String DataHeader = "Time ,ProbeTemp , InternalTemp , InternalHum , ElecTemp , ElecHumidity , Pressure"; // for the CSV for data logger
const String DebugHeader = "Time , Sample Count , SD Count  , DataTocloud , increment, Reconnects";   // for the CSV for Debug messages
...
void Task1code( void * pvParameters ){
  while (true){
    Serial.println("Data to Cloud thread started");
...
Serial.println("Data to Cloud thread ended");
...
void Task2code( void * pvParameters ){
  while (true){
    Serial.println("Sensor to CSV thread started");
    ...
    Serial.println("Sensor to CSV thread ended");
...
const String DebugHeader = "Time , Sample Count , SD Count  , DataTocloud , increment, Reconnects";   // for the CSV for Debug messages
  ...
  Serial.println("--in DataToCSV:");      // indicate routine we are in
  // Writes data to SD card in CSV format
  CSVdata = String(millis()) + "," + String(temperatureC) + ","+ String(SiTemp) + ","+ String(SiHum); 
  CSVdata = CSVdata + String(Temperature) + "," + String(Humidity) + ","+ String(pressureV);
  CSVdata.toCharArray(dataStr,100);   // convert string to char array
  // SD card send data 
  //appendFile(SD_MMC, "/datalogger.csv", dataStr);
  Serial.println(DataHeader);
  Serial.println(dataStr);
  Serial.println("XXout of DataToCSV:");      // indicate routine we are in
 
  msgs_card ++ ;      // one more msg written to card.
}
...
const String DebugHeader = "Time , Sample Count , SD Count  , DataTocloud , increment, Reconnects";   // for the CSV for Debug messages
  ...
  Serial.println("--in DebugToCSV:");      // indicate routine we are in
  //write debug data to CS file.
  String CSVdata = String(msgs_sensors) + "," + String(msgs_card) + ","+ String(msgs_cloud) + ","+ String(Reconnects); 
  CSVdata.toCharArray(dataStr,100);     // convert string to char array
  // SD card send data 
//  appendFile(SD_MMC, "/debuglogger.csv", dataStr);
  Serial.println(DebugHeader);
  Serial.println(dataStr);
  Serial.println("XXout of DebugToCSV:");      // indicate routine we are in
}
...
void DataToCloud() {
  ...
  Serial.println("in DataToCloud:");      // indicate routine we are in
...
*/
/*
void ReadDeviceLabel(fs::FS &fs, const char * path){
    Serial.printf("Reading file: %s\n", path);
...
void appendFile(fs::FS &fs, const char * path, const char * message){
    Serial.println("--in appendFile:");      // indicate routine we are in
    Serial.printf("Appending to file: %s\n", path);
    Serial.printf("data to append: %s\n", message);
    ...
    Serial.println("XXout of appendFile:");      // indicate routine we are in
}
...
void setup() {
  ...
  Serial.println("Start Setup:");   // indicate where we are
...
*/
  ...
  if (totalStorage > 1.1*usedStorage) {
    DataHeader.toCharArray(dataStr,100);     // convert string to char array
    //writeFile(SD_MMC, "/datalogger.csv", dataStr);
    Serial.print("SD_MMC Card DataLogger.csv : ");
    Serial.print(dataStr);
    Serial.print("  Done  ");
    DebugHeader.toCharArray(dataStr,100);     // convert string to char array
    //writeFile(SD_MMC, "/debuglogger.csv", dataStr);
